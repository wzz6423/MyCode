...
#include <cpr/cpr.h>
// json
#include "../utils/utils.hpp" // utils 模块封装
// Other
    ...
    namespace ElasticSearch
    {
        ...
        public:
            ...
            auto Create(const std::string &indexId = "defaultIndexId") -> bool
            {
                ...
                std::optional<std::string> jsonIndex_opt = Utils::JsonUtil::Serialize(_index);
                if (!jsonIndex_opt.has_value()) [[unlikely]]
                {
                    Log::lg("error", Log::FileName(), Log::Line(),
                            "index serialize failed!");
                    return false;
                }
                else
                {
                    std::string jsonIndex = std::move(jsonIndex_opt.value());
                    // 要进行异常捕获, 防止进程崩溃
                    try
                    {
                        // 发起请求
        ...
        public:
            ...
            {
                std::optional<std::string> jsonItem_opt = Utils::JsonUtil::Serialize(_item);
                if (!jsonItem_opt.has_value()) [[unlikely]]
                {
                ...
                {
                    std::string jsonItem = std::move(jsonItem_opt.value());
                    // 要进行异常捕获, 防止进程崩溃
                    try
                    {
                        // 发起请求
        ...
        public:
            ...
            auto Search() -> std::optional<Json::Value>
            {
                ...
                std::optional<std::string> jsonSearch_opt = Utils::JsonUtil::Serialize(root);
                cpr::Response rsp;
                if (!jsonSearch_opt.has_value())
                {
                    Log::lg("error", Log::FileName(), Log::Line(), "item serialize error!");
                    return std::nullopt;
                }
                ...
                {
                    std::string jsonSearch = std::move(jsonSearch_opt.value());
                    // 要进行异常捕获, 防止进程崩溃
                    try
                    {
                        // 发起请求
                ...
                // 反序列化响应正文
                std::optional<Json::Value> jsonResponse_opt = Utils::JsonUtil::UnSerialize(rsp.text);
                if (!jsonResponse_opt.has_value())
                {
                    Log::lg("error", Log::FileName(), Log::Line(), "unserialize {} error!", rsp.text);
                    return std::nullopt;
                }
                Json::Value jsonResponse = std::move(jsonResponse_opt.value());
