...
public:
    // 获取时间戳
    static time_t get_now_time()
    {
        return time(nullptr);
    }

    // 获取格式化的时间
    static std::string get_formatted_time(const char *format = "%Y-%m-%d %H:%M:%S")
    {
    ...
    // 通过文件名从某路径下查找并获取完整文件路径
    static std::optional<std::filesystem::path> get_file_path(const std::string &file_name, const std::filesystem::path &dir = std::filesystem::current_path(), size_t max_depth = 6)
    {
        try
        {
            for (auto it = std::filesystem::recursive_directory_iterator(dir, std::filesystem::directory_options::skip_permission_denied); it != std::filesystem::recursive_directory_iterator(); ++it)
            {
    ...
    static std::string extract_directory(const std::string &full_path)
    {
    ...
    // 检测某路径下某文件的存在性
    static bool is_file_exists(const std::string &file_name, const std::filesystem::path &dir = std::filesystem::current_path())
    {
        if (get_file_path(file_name, dir) == std::nullopt)
        {
            return false;
        }
        return true;
    ...
    // 创建文件，若路径不存在会自动创建目录
    static bool create_file(const std::string &file_path, const std::string &content = "")
    {
        ...
        file << content; // 写入内容（可选）
        file.close();
        return true;
    }

    // 在文件中写入内容，可以选择覆盖写/追加写，支持使用容器
    template <typename Container>
    bool safe_write_lines(const std::string &file_path, const Container &lines, bool append = false)
    {
        // 1. 检查并创建父目录
        if (!is_file_exists(extract_filename(file_path), extract_directory(file_path)))
        {
